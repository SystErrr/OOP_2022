...
def input_int():  # Ввод числа и перевод его в тип int
    s_number = int(input())
    return s_number


def input_many():  # Ввод нескольких чисел, перевод их в тип int и занесение их в массив
    m_numbers = []
    m_numbers = list(map(int, input().split()))
    return m_numbers


def angry_emoji():  # рисует злой смайлик
...
def window_scale(x, y, i, screen):  # рисует окно с параметрами
    i = i / 10
    x0 = 415
    y0 = 10
    x1 = - x0 * i + x
...
def cat_scale(x, y, i, screen, color):  # рисует кота с параметрами
    i = i / 10
    x0 = 20
    y0 = 370
    x1 = - x0 * i + x
...
def pygame_done():  #  основные функции работы pygame и ее завершение
